...
public class BossGuidesApiControllerTests
{
    ...
    // GET /api/boss-guides?q=Ragnaros — returns only matching bosses
    [Fact]
    public async Task GetAll_FilterByQ_ReturnsOnlyMatchingBosses()
    {
        var dbName = Guid.NewGuid().ToString();
        await using var factory = CreateFactory(dbName);
        ...
        using (var scope = factory.Services.CreateScope())
        {
            var db = scope.ServiceProvider.GetRequiredService<ApplicationDbContext>();
            var raid = SeedRaid(db);
            SeedBoss(db, raid.Id); // "Test Boss"
            db.BossGuides.Add(new BossGuide
            {
                BossName = "Ragnaros",
                Tactics = "Stand back",
                BossImageUrl = "https://example.com/rag.jpg",
                DifficultyRating = 8,
                RaidGuideId = raid.Id
            });
            db.SaveChanges();
        ...
        var client = factory.CreateClient();
        var response = await client.GetAsync("/api/boss-guides?q=Ragnaros");

        Assert.Equal(HttpStatusCode.OK, response.StatusCode);
        var list = await response.Content.ReadFromJsonAsync<List<BossGuideDto>>(JsonOptions);
        Assert.NotNull(list);
        Assert.Single(list);
        Assert.Equal("Ragnaros", list[0].BossName);
    }

    // GET /api/boss-guides?raidGuideId=X — returns only bosses for that raid
    [Fact]
    public async Task GetAll_FilterByRaidGuideId_ReturnsOnlyMatchingBosses()
    {
        var dbName = Guid.NewGuid().ToString();
        await using var factory = CreateFactory(dbName);
        ...
        {
            var db = scope.ServiceProvider.GetRequiredService<ApplicationDbContext>();
            var raid1 = SeedRaid(db);
            SeedBoss(db, raid1.Id);
            var raid2 = new RaidGuide { RaidName = "Second Raid", PreparationNotes = "" };
            db.RaidGuides.Add(raid2);
            db.SaveChanges();
            raid2Id = raid2.Id;
            db.BossGuides.Add(new BossGuide
            {
        ...
        var client = factory.CreateClient();
        var response = await client.GetAsync($"/api/boss-guides?raidGuideId={raid2Id}");

        Assert.Equal(HttpStatusCode.OK, response.StatusCode);
        var list = await response.Content.ReadFromJsonAsync<List<BossGuideDto>>(JsonOptions);
        Assert.NotNull(list);
        Assert.Single(list);
        Assert.Equal(raid2Id, list[0].RaidGuideId);
    }

    // GET /api/boss-guides?minDifficulty=7 — returns only bosses with difficulty >= 7
    [Fact]
    public async Task GetAll_FilterByMinDifficulty_ReturnsOnlyHardBosses()
    {
        var dbName = Guid.NewGuid().ToString();
        await using var factory = CreateFactory(dbName);
        ...
        using (var scope = factory.Services.CreateScope())
        {
            var db = scope.ServiceProvider.GetRequiredService<ApplicationDbContext>();
            var raid = SeedRaid(db);
            SeedBoss(db, raid.Id); // DifficultyRating = 5
            db.BossGuides.Add(new BossGuide
            {
                BossName = "Hard Boss",
                Tactics = "Very hard",
                BossImageUrl = "https://example.com/hard.jpg",
                DifficultyRating = 9,
                RaidGuideId = raid.Id
            });
            db.SaveChanges();
        ...
        var client = factory.CreateClient();
        var response = await client.GetAsync("/api/boss-guides?minDifficulty=7");

        Assert.Equal(HttpStatusCode.OK, response.StatusCode);
        var list = await response.Content.ReadFromJsonAsync<List<BossGuideDto>>(JsonOptions);
        Assert.NotNull(list);
        Assert.All(list, item => Assert.True(item.DifficultyRating >= 7));
    }

    // GET /api/boss-guides?maxDifficulty=5 — returns only bosses with difficulty <= 5
    [Fact]
    public async Task GetAll_FilterByMaxDifficulty_ReturnsOnlyEasyBosses()
    {
        var dbName = Guid.NewGuid().ToString();
        await using var factory = CreateFactory(dbName);
        ...
        var client = factory.CreateClient();
        var response = await client.GetAsync("/api/boss-guides?maxDifficulty=5");

        Assert.Equal(HttpStatusCode.OK, response.StatusCode);
        var list = await response.Content.ReadFromJsonAsync<List<BossGuideDto>>(JsonOptions);
        Assert.NotNull(list);
        Assert.All(list, item => Assert.True(item.DifficultyRating <= 5));
    }

    // POST /api/boss-guides — returns 400 when RaidGuideId does not exist in DB
    [Fact]
    public async Task Post_Returns400_WhenRaidGuideIdNotFound()
    {
        var dbName = Guid.NewGuid().ToString();
        await using var factory = CreateFactory(dbName);
        ...
        var dto = new BossGuideUpsertDto
        {
            BossName = "Orphan Boss",
            Tactics = "None",
            BossImageUrl = "https://example.com/orphan.jpg",
            DifficultyRating = 5,
            RaidGuideId = 99999
        };
    ...
    // PUT /api/boss-guides/{id} — returns 400 when RaidGuideId does not exist in DB
    [Fact]
    public async Task Put_Returns400_WhenRaidGuideIdNotFound()
    {
        var dbName = Guid.NewGuid().ToString();
        await using var factory = CreateFactory(dbName);
